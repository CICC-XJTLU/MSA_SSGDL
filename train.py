...
def parser_args():
    ...
    parser.add_argument("--data_path", type=str, default='/root/autodl-tmp/SSGDL/dataset/MOSI_16_sentilare_unaligned_data.pkl')
    ...
    parser.add_argument("--model_name_or_path", default='/root/autodl-tmp/SSGDL/pretrained_model/sentilare_model/sentilare_model/', type=str,   
...
def prep_for_training(args, num_train_optimization_steps: int):
    ...
    optimizer_grouped_parameters = [
        {
            "params": [
                 p for n, p in param_optimizer if not any(nd in n for nd in no_decay)  and not any(nd in n for nd in SSGDL_params)
            ],
            "weight_decay": args.weight_decay,
        },
        {"params": model.roberta.encoder.SSGDL.parameters(), 'lr':args.learning_rate, "weight_decay": args.weight_decay},
        {
            "params": [
                p for n, p in param_optimizer if any(nd in n for nd in no_decay)  and not any(nd in n for nd in SSGDL_params)
            ],
            "weight_decay": 0.0,
        },
    ]
...
def train(
    ...
    for epoch_i in range(int(args.n_epochs)):
        ...
        # print(
        #     "epoch:{}, train_loss:{}, train_acc:{}, test_loss:{}, test_acc:{}".format(
        #         epoch_i, train_loss, train_acc, test_loss, test_acc
        #         )
        #     )
        ...
        file_path = '/root/autodl-tmp/SSGDL/saved_models/SSGDL.pth'
        torch.save(model.state_dict(), file_path)


def main():
    args = parser_args()
    wandb.init(project="SSGDL", reinit=True)
